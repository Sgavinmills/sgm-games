  ...
  function validateBody(body) {
    if (body.length < 20 || body.length > 2000) {
      setFormErrors((currErrors) => {
        ...
        newErrors.commentBody.msg =
          "Comment body must be between 20-2000 chars";
        return newErrors;
      });
    } else
      setFormErrors((currErrors) => {
    ...
    postComment(postData, review_id)
      .then((response) => {
        setCommentsList((currComments) => {
          const newComments = currComments.map((commentObj) => {
            return { ...commentObj };
          });
      ...
      <section className={styles["comment-form-container"]}>
        ...
        >
          <input
            ...
            className={styles["review-comment-body"]}
            type="text"
            id="title"
            name="title"
            placeholder="Enter comment here.... 2000 words max"
          />
          {formErrors.commentBody.error && (
            <p className={styles["error-paragraph"]}>
              {formErrors.commentBody.msg}
            </p>
          )}
