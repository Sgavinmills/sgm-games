    ...
    <div>
      <div className={styles["pagination-container"]}>
        <p className={styles.pagination}>
          ...
          >
            {filters.p < Math.ceil(totalItems / filters.limit) && (
      ...
      >
        <select defaultValue={filters.limit}>
          <option value={filters.limit}>
            Max results per page: {filters.limit}
          </option>
          {totalItems >= 3 && (
            <option value={3}>Max results per page: 3</option>
          )}
          {totalItems >= 5 && (
            <option value={5}>Max results per page: 5</option>
          )}
          {totalItems >= 10 && (
            <option value={10}>Max results per page: 10</option>
          )}
          {totalItems >= 20 && (
            <option value={20}>Max results per page: 20</option>
          )}
          {totalItems >= 30 && (
            <option value={30}>Max results per page: 30</option>
          )}
          {totalItems >= 50 && (
            <option value={50}>Max results per page: 50</option>
          )}
        </select>
      </form>
    </div>
