...
export default function Filters({ filters, setFilters, categories }) {
  ...
  const goToReviewByCategory = (category) => {
  ...
  return (
    <div>
      <div
        ...
        onClick={() => {
          setShowFilterContainer((curr) => !curr);
        }}
      >
      ...
      {showFilterContainer && (
        <div className={styles["filter-container"]}>
          <form>
            <select
              onChange={(event) => {
                goToReviewByCategory(event.target.value);

                setFilters((currFilter) => {
                  const newFilter = { ...currFilter };
                  newFilter.category = categories.some(
                    (cat) => cat.slug === event.target.value
                  )
                    ? event.target.value
                    : null;
                  return newFilter;
            ...
            >
              <option value={null}>
                {filters.category ? filters.category : "Category: all"}
              </option>
            ...
            >
              <option value="">
                {filters.sort_by === "owner" && filters.order === "asc"
                  ? "Reviewed by (A-Z)"
                  : filters.sort_by === "owner" && filters.order === "desc"
                  ? "Reviewed by (Z-A)"
                  : filters.sort_by === "title" && filters.order === "asc"
                  ? "Title (A-Z)"
                  : filters.sort_by === "title" && filters.order === "desc"
                  ? "Title (Z-A)"
                  : filters.sort_by === "category"
                  ? "Category"
                  : filters.sort_by === "comment_count" &&
                    filters.order === "asc"
                  ? "Comments (low - high)"
                  : filters.sort_by === "comment_count" &&
                    filters.order === "desc"
                  ? "Comments (high - low)"
                  : filters.sort_by === "created_at" && filters.order === "asc"
                  ? "Date reviewed - Oldest first"
                  : filters.sort_by === "created_at" && filters.order === "desc"
                  ? "Date reviewed - Newest first"
                  : filters.sort_by === "votes" && filters.order === "asc"
                  ? "Votes (low - high)"
                  : filters.sort_by === "votes" && filters.order === "desc"
                  ? "Votes (high - low"
                  : "Sort By: none"}
              </option>
