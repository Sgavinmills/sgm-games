...
export default function ReviewsVotes({
  setErr,
  likedReviews,
  reviewObj,
  loggedInUser,
    ...
    <>
      <span
        style={{
          color:
            likedReviews.some(
              (review) => review.review_id === reviewObj.review_id
            ) &&
            likedReviews.find(
              (review) => review.review_id === reviewObj.review_id
            ).vote_type === "down"
        ...
        onClick={() => {
          changeVotes(reviewObj.review_id, "down", loggedInUser);
        }}
        className={styles["votes-thumbs"]}
      >
      ...
      <span
        ...
        className={styles["votes-thumbs"]}
        onClick={() => {
          changeVotes(reviewObj.review_id, "up", loggedInUser);
        }}
      >
        {" "}
  ...
  function changeVotes(review_id, vote_type, loggedInUser) {
    const theLikedReview = likedReviews.find(
      (review) => review.review_id === review_id
    );
    const thisReview = reviewsList
      ? reviewsList.find((review) => review.review_id === review_id)
      : review;
    if (
      (thisReview.votes === 0 && vote_type === "down") ||
      (theLikedReview && vote_type === "down" && thisReview.votes === 1)
    ) {
    } else {
      if (!theLikedReview) {
        //if its not in the liked review list then stick in in the likedReview list
        setLikedReviews((currLikedReviews) => {
          const newLikedReviews = currLikedReviews.map((review) => {
            return { ...review };
          });
        ...
        if (setReviewsList) {
          setReviewsList((currReviews) => {
            const newReviews = currReviews.map((review) => {
              return { ...review };
            });
            const reviewToEdit = newReviews.find(
              (review) => review.review_id === review_id
            );
            reviewToEdit.votes =
              vote_type === "up"
                ? reviewToEdit.votes + 1
                : reviewToEdit.votes - 1;
            return newReviews;
          });
        } else {
          setReview((currReview) => {
            ...
            newReview.votes =
              vote_type === "up" ? newReview.votes + 1 : newReview.votes - 1;
            return newReview;
          });
        }
      } else {
        ...
        if (theLikedReview.vote_type === vote_type) {
          setLikedReviews((currLikedReviews) => {
            ...
            const theLikedReviewIndex = newLikedReviews.findIndex(
              (review) => review.review_id === review_id
            );
          ...
          setLikedReviews((currLikedReviews) => {
            ...
            const theLikedReview = newLikedReviews.find(
              (review) => review.review_id === review_id
            );
            theLikedReview.vote_type =
              theLikedReview.vote_type === "up" ? "down" : "up";
            return newLikedReviews;
          });
          if (reviewsList) {
            setReviewsList((currReviews) => {
            ...
            setReview((currReview) => {
              const newReview = { ...currReview };
              newReview.votes =
                vote_type === "up" ? newReview.votes + 2 : newReview.votes - 2;
              return newReview;
            });
          }
        }
