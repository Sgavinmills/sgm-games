...
export default function EditCommentsForm({
  setTotalItems,
  setErr,
  setEdittingComment,
  setCommentsList,
  ...
  function submitForm() {
    ...
    setCommentsList((currCommentList) => {
      const newCommentList = currCommentList.map((comment) => {
        return { ...comment };
      });
      const edittedComment = newCommentList.find(
  ...
  return (
    ...
    >
      <input
        className={styles["form-edit-body"]}
        type="text"
      ...
      {formErrors.commentBody.error && (
        <p className={styles["error-paragraph"]}>
          Comment must be between 20 and 2000 chars
        </p>
      )}
