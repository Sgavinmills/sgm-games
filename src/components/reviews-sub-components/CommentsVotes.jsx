...
export default function CommentsVotes({
    ...
    <p className={styles["info-text"]}>
      <span
        style={{
          color:
            likedComments.some(
              (comment) => comment.comment_id === commentObj.comment_id
            ) &&
            likedComments.find(
              (comment) => comment.comment_id === commentObj.comment_id
            ).vote_type === "down"
        ...
        onClick={() => {
          changeCommentVotes(commentObj.comment_id, "down", loggedInUser);
        }}
      ...
      <span
        ...
        className={styles["votes-thumbs"]}
        onClick={() => {
          changeCommentVotes(commentObj.comment_id, "up", loggedInUser);
        }}
      >
        {" "}
  ...
  function changeCommentVotes(comment_id, vote_type, loggedInUser) {
    const theLikedComment = likedComments.find(
      (comment) => comment.comment_id === comment_id
    );
    const thisComment = commentsList.find(
      (comment) => comment.comment_id === comment_id
    );
    if (
      (thisComment.votes === 0 && vote_type === "down") ||
      (theLikedComment && vote_type === "down" && thisComment.votes === 1)
    ) {
    } else {
      if (!theLikedComment) {
        //if its not in the liked review list then stick in in the likedReview list
        setLikedComments((currLikedComments) => {
          const newLikedComments = currLikedComments.map((comment) => {
            return { ...comment };
          });
        ...
        setCommentsList((currComments) => {
          ...
          const commentToEdit = newComments.find(
            (comment) => comment.comment_id === comment_id
          );
          commentToEdit.votes =
            vote_type === "up"
              ? commentToEdit.votes + 1
              : commentToEdit.votes - 1;
          return newComments;
        });
      } else {
        ...
        if (theLikedComment.vote_type === vote_type) {
          setLikedComments((currLikedComments) => {
            ...
            const theLikedCommentIndex = newLikedComments.findIndex(
              (comment) => comment.comment_id === comment_id
            );
          ...
          setLikedComments((currLikedComments) => {
            ...
            const theLikedComment = newLikedComments.find(
              (comment) => comment.comment_id === comment_id
            );
            theLikedComment.vote_type =
              theLikedComment.vote_type === "up" ? "down" : "up";
            return newLikedComments;
          });
          setCommentsList((currComments) => {
            const newComments = currComments.map((comment) => {
            ...
            commentToEdit.votes =
              vote_type === "up"
                ? commentToEdit.votes + 2
                : commentToEdit.votes - 2;
            return newComments;
          });
        }
      }
      patchCommentVotes(comment_id, vote_type, loggedInUser.username).catch(
        (e) => {
          setErr({
