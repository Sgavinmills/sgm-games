...
export default function EditReviewsForm({
  setErr,
  setEdittingReview,
  setReviewsList,
  reviewObj,
  ...
  function submitForm() {
    ...
    if (setReviewsList) {
      setReviewsList((currReviewList) => {
        const newReviewList = currReviewList.map((review) => {
          return { ...review };
        });
        const edittedReview = newReviewList.find(
    ...
    >
      ...
      {formErrors.reviewBody.error && (
        <p className={styles["error-paragraph"]}>
          Review must be between 20 and 2000 chars
        </p>
      )}
      <div>
        <br />
