  ...
  return (
    <div>
      <section className={styles["form-container"]}>
        ...
        <form
          className={styles["add-review-form"]}
          onSubmit={(event) => {
        ...
        >
          <div className={styles["label-input-container"]}>
            ...
            <input
              ...
              type="text"
              name="title"
              placeholder="The title of the game you are going to review..."
            />
          </div>
          {formErrors.title.error && (
            <p className={styles["error-paragraph"]}>{formErrors.title.msg}</p>
          )}
          ...
          <div className={styles["label-input-container"]}>
            ...
            <input
              ...
              onChange={(event) => {
                validateDesigner(event.target.value);
              }}
              type="text"
              name="designer"
              placeholder="Game designer. Leave blank if you aren't sure :)"
            />
          </div>
          {formErrors.designer.error && (
            <p className={styles["error-paragraph"]}>
              {formErrors.designer.msg}
            </p>
          )}
          ...
          <div className={styles["label-input-container"]}>
            ...
            <input
              id="body"
              value={postData.reviewBody}
              onChange={(event) => {
                validateBody(event.target.value);
              }}
              type="text"
              name="review_body"
              placeholder="Review body... 2000 words max. Make it fun."
              className={styles["review-body"]}
            />
          </div>
          {formErrors.reviewBody.error && (
            <p className={styles["error-paragraph"]}>
              {formErrors.reviewBody.msg}
            </p>
          )}
          ...
          <div className={styles["label-input-container"]}>
            ...
            <input
              value={postData.review_img_url}
              onChange={(event) => {
                validateImgUrl(event.target.value);
              }}
              type="text"
              id="image_url"
              name="image_url"
            ...
            />
            {formErrors.review_img_url.error && (
              <p className={styles["error-paragraph"]}>
                {formErrors.review_img_url.msg}
              </p>
            )}
          </div>
          {formErrors.review_img_url.error && (
